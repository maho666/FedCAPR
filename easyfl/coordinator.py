...
class Coordinator(object):
    ...
    def init_conf(self, conf):
        ...
        """
        self.conf = conf
        self.conf.is_distributed = (self.conf.gpu > 2)
        if self.conf.gpu == 0:
            self.conf.device = "cpu"
        elif self.conf.gpu == 1:
            self.conf.device = "cuda:0"
        elif self.conf.gpu == 2:
            self.conf.device = "cuda:1"
        else:
...
def init_logger(log_level, conf):
    ...
    # file_path = path.join(file_path, conf.task_id + ".log")
    file_path = path.join(conf.save_log_path, conf.task_id + ".log")
...
def init(conf=None, init_all=True):
    ...
    #######################################################################################################################
    save_info_path = os.path.join(config.save_root, config.task_id)
    save_model_path = os.path.join(save_info_path, "saved_model")
    ...
    config.client.save_figure_path = save_info_path
    config.server.save_model_path = save_model_path
    config.save_log_path = save_info_path
    #######################################################################################################################

    # init_logger(config.tracking.log_level)
    init_logger(config.tracking.log_level, config)
