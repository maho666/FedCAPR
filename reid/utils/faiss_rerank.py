...
def faiss_compute_jaccard_dist(target_features, weight, k1=20, k2=6, print_flag=True, search_option=0, use_float16=False):
    end = time.time()
    if print_flag:
    ...
    # expended code for dataset append version ###############
    # deducted_length = int(N / weight)
    # deducted_features = torch.zeros((deducted_length, 2048), dtype = target_features.dtype)
    # for i in range(deducted_length):
    #     base = i * weight
    #     deducted_features[i, :] = torch.mean(target_features[base:(base+weight), :], dim=0)
    ##########################################################
    ...
    if (search_option==0):
        ...
        # _, initial_rank = search_raw_array_pytorch(res, deducted_features, deducted_features, k1)
...
######################### original code########################################
    ...
    #     k_reciprocal_expansion_index = np.unique(k_reciprocal_expansion_index)  ## element-wise unique
    #     dist = 2-2*torch.mm(deducted_features[i].unsqueeze(0).contiguous(), deducted_features[k_reciprocal_expansion_index].t())
    #     if use_float16:
    #         V[i,k_reciprocal_expansion_index] = F.softmax(-dist, dim=1).view(-1).cpu().numpy().astype(mat_type)
    #     else:
    #         V[i,k_reciprocal_expansion_index] = F.softmax(-dist, dim=1).view(-1).cpu().numpy()
